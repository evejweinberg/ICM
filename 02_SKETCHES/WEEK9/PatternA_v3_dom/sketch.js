...
var roveBothax, roveBothay, roveBothbx, roveBothby, wiggleaway;
...
function preload() { //load all images here
  ...
  for (var i = 0; i < 7; i++) {
    abg = "assets/Abg_000" + i + ".png"; //store the name of the images
    abgAllDom.push(createImg(abg)); //push into DOM elements
...
// abgAllDom[5].position(0,0).mouseOver(wiggle);
...
function wiggle(){
  console.log("wiggle!!")
  wiggleaway = random(-5,5);
  //scoot in a stop-motion way, away from mouse
}
...
function draw() {
  background(255, 0, 0);
bgCollage(0+wiggleaway,0+wiggleaway);
...
// abgAllDom[0].position(200, 300).mouseOver(wiggle);


  if (transition === true) {
   abgAllDom[0].position(200+wiggleaway, 300+wiggleaway).mouseOver(wiggle);
     abgAllDom[1].position(50+wiggleaway, 100+wiggleaway).mouseOver(wiggle);
      abgAllDom[2].position(0+wiggleaway, 500+wiggleaway).mouseOver(wiggle);
       abgAllDom[3].position(100+wiggleaway, 200+wiggleaway).mouseOver(wiggle);
