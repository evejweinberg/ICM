...
function preload() {
  img = loadImage("assets/BlackGrid.jpg");
...
function draw() {
  ...
  var newRing = new Ring(mouseX + random(-170, 170), mouseY + random(-170, 170));
  rings.push(newRing);
  var newSquare = new Square(mouseX + random(-300, 430), mouseY + random(-300, 430), 100, 100, 30, 30);
  ...
  for (var i = 0; i < squares.length; i++) {
    if (dist(mouseX, mouseY, squares[i].x, squares[i].y) < distance) {
...
function Ring(x, y) {
  this.x = x;
  this.y = y;
  this.external_radius = random(140);
